...
* Which of [MLOntology_Explanation.ttl](Protege_Ontologies/MLOntology_Explanation.ttl) and [MLExplanationOntology.owl.ttl](Protege_Ontologies/MLExplanationOntology.owl.ttl) is the reference one? Running each through `rapper -i turtle -o turtle $IN > $OUT` which sorts the files and groups them together shows a lot of overlap, but no clear winner.
